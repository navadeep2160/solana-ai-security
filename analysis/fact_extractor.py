...
class FactExtractor:
    ...
    def _extract_cfg_facts(self, code: str, cfg_result) -> List[SecurityFact]:
        ...
        if cfg_result and isinstance(cfg_result, dict):
            for func_name, func_data in cfg_result.get("functions", {}).items():
                for block in func_data.get("blocks", []):
                    ...
                    fact = SecurityFact(
                        ...
                        context={
                            "signer_check": tags.get("signer_check", True),
                            "owner_check": tags.get("owner_check", True),
                            "unchecked_math": tags.get("unchecked_math", False),
                            "transfer": tags.get("transfer", False),
                            "close": tags.get("close", False),
                            "init": tags.get("init", False),
                            "cpi": tags.get("cpi", False),
