...
class KBPatternMatcher:
        ...
        for node in self.vuln_nodes:
            ...
            if score >= 15:
                matches.append({
                    "vuln_type": node["name"],
                    "category": category,
                    "severity": self._adjust_severity(node.get("severity", "medium"), score),
                    "function": function,
                    "line": line,
    ...
    def _adjust_severity(self, base_severity: str, score: int) -> str:
        """Adjust severity based on match confidence."""
        # Only promote to CRITICAL if very high confidence
        if base_severity.lower() == "critical" and score < 35:
            return "HIGH"
        if base_severity.lower() == "high" and score < 20:
            return "MEDIUM"
        if base_severity.lower() == "medium" and score < 15:
            return "LOW"
